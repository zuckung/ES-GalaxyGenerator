...
def write_readme():
	...
	with open('README.md', 'w') as target:
		...
		for each in folder:
			...
			races = lines[4].strip()
			wormhole = lines[5].strip()
			sunmax = lines[6].strip()
			planetmax = lines[7].strip()
			minablemax = lines[8].strip()
			galaxyImage = lines[9].strip()
			starRadius = lines[10].strip()
			density = lines[11].strip()
			tries = lines[12].strip()
